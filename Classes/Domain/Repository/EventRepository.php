...
class Tx_RoqNewsevent_Domain_Repository_EventRepository extends \GeorgRinger\News\Domain\Repository\NewsRepository {
	...
	protected function createIsActiveConstraint(\TYPO3\CMS\Extbase\Persistence\QueryInterface $query) {
		...
		$simpleQuery = $queryBuilder
			...
			->andWhere(
				// future events
				"(tx_roqnewsevent_startdate + tx_roqnewsevent_starttime > $timestamp) OR" .
				// current multiple day events
				"(tx_roqnewsevent_startdate + tx_roqnewsevent_starttime < $timestamp AND tx_roqnewsevent_enddate + tx_roqnewsevent_endtime > $timestamp) OR" .
				// current single day events
				"(tx_roqnewsevent_startdate + tx_roqnewsevent_starttime < $timestamp AND tx_roqnewsevent_startdate + tx_roqnewsevent_endtime > $timestamp AND tx_roqnewsevent_enddate = 0) OR" .
				// current single day event without time
				"(tx_roqnewsevent_startdate + 86399 > $timestamp AND tx_roqnewsevent_starttime = 0 AND tx_roqnewsevent_enddate = 0 AND tx_roqnewsevent_endtime = 0)"
			);
		$result = $simpleQuery->execute();
		$newsUids =  [];
		...
		if (count($newsUids) > 0) {
			$constraint = $query->in('uid', $newsUids);
			return $constraint;
		}
	}
